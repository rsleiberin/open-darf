...
cd "$ROOT"
OUTDIR="var/receipts/phase7g/plmarker_runs/$(date -u +%Y%m%d-%H%M%S)"
mkdir -p "$OUTDIR"

# 1) Run runner (may be deps_missing)
python3 apps/relex/plmarker/runner.py --dataset_dir data/scierc_norm --split dev --outdir "$OUTDIR" || true

# 2) If no metrics, bridge from best heuristic baseline
...
best["model"]=best.get("model","?")+ "+PLMarker-bridge"
best["notes"]=(best.get("notes","")+"; imported into PL-Marker outdir via shim").strip("; ")
