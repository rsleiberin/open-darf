...
if (-not $composeCmd) {
    Write-Host "⚠️  WARNING: Docker Compose not found" -ForegroundColor Yellow
...
if ($composeCmd) {
    try {
        if ($KeepVolumes) {
            Write-Host "ℹ️  Keeping data volumes (use without -KeepVolumes to remove)" -ForegroundColor Cyan
            ...
            Write-Host "⚠️  Removing data volumes (all data will be lost)" -ForegroundColor Yellow
            if ($composeCmd -eq "docker-compose") {
        ...
        Write-Host "⚠️  Compose cleanup failed, trying manual cleanup" -ForegroundColor Yellow
    }
}
...
try {
    ...
    if ($remainingContainers) {
        Write-Host "⚠️  Some DARF containers still exist:" -ForegroundColor Yellow
    ...
    Write-Host "ℹ️  Could not verify container cleanup" -ForegroundColor Cyan
}
...
try {
    ...
    if ($remainingNetworks) {
        Write-Host "⚠️  Some DARF networks still exist:" -ForegroundColor Yellow
    ...
    Write-Host "ℹ️  Could not verify network cleanup" -ForegroundColor Cyan
}
...
if (-not $KeepVolumes) {
    try {
        ...
        if ($remainingVolumes) {
            Write-Host "⚠️  Some DARF volumes still exist:" -ForegroundColor Yellow
        ...
        Write-Host "ℹ️  Could not verify volume cleanup" -ForegroundColor Cyan
    }
}
