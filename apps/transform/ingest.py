...
import argparse, hashlib, json, mimetypes, os, sys, time, uuid
...
def ingest(file_path: str, user_id: str, auth_class: str = "owner", doc_type: str | None = None) -> Dict[str, Any]:
    fp = Path(file_path)
    ...
    manifest_bytes = build_manifest_v1(doc_bytes, mime, spans, user_id=user_id, auth_class=auth_class)
...
# --- CLI ---
def main(argv=None):
    ap = argparse.ArgumentParser(description="Ingest a file → CAS + manifest + draft.json (pointer-only).")
    ap.add_argument("--file", required=True, help="Path to source file")
    ap.add_argument("--user", required=True, help="User id")
    ap.add_argument("--auth-class", default="owner", choices=["owner","trusted","untrusted"], help="Provenance class")
    ap.add_argument("--doc-type", default=None, help="Override MIME type")
    args = ap.parse_args(argv)

    t0 = time.perf_counter()
    draft = ingest(args.file, args.user, auth_class=args.auth_class, doc_type=args.doc_type)
    elapsed_ms = (time.perf_counter() - t0) * 1000.0

    # Minimal operator-friendly output
    print(json.dumps({"result":"ok","elapsed_ms": round(elapsed_ms,2), "draft": draft}, ensure_ascii=False, indent=2))
