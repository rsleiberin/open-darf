...
def _embed3(text: str) -> list[float]:
    L=len(text); s=text.count(" "); a=sum(ch.isalnum() for ch in text)
...
def _audit(event: str, data: Dict[str, Any]) -> None:
    ...
    with open(AUDIT_DIR/f"{_now_ms()}_{event}.jsonl","a") as f: f.write(json.dumps(rec)+"\n")
...
def ingest(file_path: str, user_id: str, intent_anchor: str|None=None) -> Path:
    ...
    draft={
        ...
        "user_id": user_id,
        "origin": {"file": str(p), "bytes": len(content), "intent_anchor": intent_anchor},
        "cas": {"digest": ref.digest, "key": f"sha256/{ref.digest[:2]}/{ref.digest[2:4]}/{ref.digest}"},
        "qdrant": {"point_id": point_id, "collection": os.getenv("QDRANT_COLLECTION","anchors")},
        "created_at_ms": _now_ms(),
        "explain": {"method": "anchors", "embedding_dim": 3}
    }
    out=DRAFT_DIR/f"{draft['draft_id']}.json"
    out.write_text(json.dumps(draft, indent=2))
    _audit("ingest", {"draft_id": draft["draft_id"], "user_id": user_id, "digest": ref.digest, "point_id": point_id})
    return out
...
def validate(draft_path: str) -> Path:
    ...
    draft["validation"]={"status":"pass","by":"placeholder","notes":"validator wiring to follow (#204)"}
...
def search(text: str, top_k: int, user_id: str|None=None):
    q=QdrantSearch()
    vec=_embed3(text)
    return [{"id": r.id, "score": r.score, "payload": r.payload} for r in q.query(vec, top_k=top_k, user_id=user_id)]
...
def validate_neo4j(draft_path: str) -> Path:
    ...
    p.write_text(json.dumps(draft, indent=2))
    _audit("validate", {"draft_id": draft["draft_id"], "status": result["status"], "latency_ms": result["latency_ms"]})
    return p
