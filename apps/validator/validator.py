...
def validate(
    ...
    *,
    ...
    result = {
        ...
        "breakdown_ms": {
            f"{io_meta.get('source')}_read+json": round(io_ms, 2),
            "neo4j_mirror": round(neo4j_ms, 2),
        },
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Validate draft against constitutional gates + Neo4j mirror."
    )
    ap.add_argument("--draft", required=True, help="Path to draft JSON")
    ap.add_argument(
        "--strict", action="store_true", help="Fail if latency exceeds target"
    )
    ap.add_argument(
        "--no-spans", action="store_true", help="Skip mirroring Span nodes for speed"
    )
    ap.add_argument(
        "--max-spans",
        type=int,
        default=None,
        help="Limit number of Span nodes mirrored",
    )
