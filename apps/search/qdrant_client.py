...
class QdrantSearch:
    ...
    def upsert(self, vector: List[float], payload: Dict[str, Any], point_id: Optional[int] = None) -> int:
    ...
    def delete_by_anchor(self, anchor_id: str) -> None:
        f = Filter(must=[FieldCondition(key="anchor_id", match=MatchValue(value=anchor_id))])
